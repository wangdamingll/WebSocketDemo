...
{
	std::string Result;
	NLMISC_CONVERT_VARGS(Result, format, STRING::MaxCStringSize);
	return Result;
}
...
inline std::string toString(const uint8_t &val) { return toString("%hu", (uint16_t)val); }
inline std::string toString(const int8_t &val) { return toString("%hd", (int16_t)val); }
inline std::string toString(const uint16_t &val) { return toString("%hu", val); }
inline std::string toString(const int16_t &val) { return toString("%hd", val); }
inline std::string toString(const uint32_t &val) { return toString("%u", val); }
inline std::string toString(const int32_t &val) { return toString("%d", val); }
inline std::string toString(const uint64_t &val) { return toString("%"NL_I64"u", val); }
inline std::string toString(const int64_t &val) { return toString("%"NL_I64"d", val); }
...
inline bool fromString(const std::string &str, uint32_t &val) { if (str.find('-') != std::string::npos) { val = 0; return false; } char *end; unsigned long v; errno = 0; v = strtoul(str.c_str(), &end, 10); if (errno || v > UINT_MAX || end == str.c_str()) { val = 0; return false; } else { val = (uint32_t)v; return true; } }
inline bool fromString(const std::string &str, int32_t &val) { char *end; long v; errno = 0; v = strtol(str.c_str(), &end, 10); if (errno || v > INT_MAX || v < INT_MIN || end == str.c_str()) { val = 0; return false; } else { val = (int32_t)v; return true; } }
inline bool fromString(const std::string &str, uint8_t &val) { char *end; long v; errno = 0; v = strtol(str.c_str(), &end, 10); if (errno || v > UCHAR_MAX || v < 0 || end == str.c_str()) { val = 0; return false; } else { val = (uint8_t)v; return true; } }
inline bool fromString(const std::string &str, int8_t &val) { char *end; long v; errno = 0; v = strtol(str.c_str(), &end, 10); if (errno || v > SCHAR_MAX || v < SCHAR_MIN || end == str.c_str()) { val = 0; return false; } else { val = (int8_t)v; return true; } }
inline bool fromString(const std::string &str, uint16_t &val) { char *end; long v; errno = 0; v = strtol(str.c_str(), &end, 10); if (errno || v > USHRT_MAX || v < 0 || end == str.c_str()) { val = 0; return false; } else { val = (uint16_t)v; return true; } }
inline bool fromString(const std::string &str, int16_t &val) { char *end; long v; errno = 0; v = strtol(str.c_str(), &end, 10); if (errno || v > SHRT_MAX || v < SHRT_MIN || end == str.c_str()) { val = 0; return false; } else { val = (int16_t)v; return true; } }
inline bool fromString(const std::string &str, uint64_t &val) { bool ret = sscanf(str.c_str(), "%"NL_I64"u", &val) == 1; if (!ret) val = 0; return ret; }
inline bool fromString(const std::string &str, int64_t &val) { bool ret = sscanf(str.c_str(), "%"NL_I64"d", &val) == 1; if (!ret) val = 0; return ret; }
